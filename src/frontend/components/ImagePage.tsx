...
const ResultsDisplay: React.FC<ResultsDisplayProps> = ({
  ...
  const [filteredMidiFiles, setFilteredMidiFiles] = useState<Result[]>([]);
      ...
      <div className="grid grid-cols-5 gap-5 w-full">
        {currentResults.map((result, index) => (
          <div
            ...
            className="rounded-xl flex flex-col items-center bg-[#1f1f1fee] pb-3 relative hover:opacity-40 group"
            onClick={
              result.midiFile
                ? () =>
                    isPlaying && currentMidi === result.midiFile
                      ? pauseMidi()
                      : playMidi(result.midiFile!, index + startIndex)
                : undefined // No onClick handler if midiFile is null
            }
          >
            <Image
              src={PlayIcon}
            ...
            />
            <img
              src={`http://127.0.0.1:8000/uploads/${datasetPictureName}/${result.filename}`}
              alt={result.filename}
            ...
            {result.mapped_audio_name ? (
              ...
              <p className="text-white mt-2 font-semibold">{result.midiFile}</p>
            )}
            ...
            {result.midiFile ? (
              ...
              <p className="text-white mt-2 text-xs">No MIDI file mapped</p>
            )}
          </div>
        ))}
      </div>
      ...
      {currentMidi && (
        <div className="bg-gradient-to-b from-[#22353a] to-[#1b2227] text-white absolute -bottom-16 w-full rounded-xl p-6">
          {results[currentMidiIndex!]?.mapped_audio_name ? (
            ...
            <h3 className="text-center font-semibold">
              {results[currentMidiIndex!]?.midiFile}
            </h3>
          )}
